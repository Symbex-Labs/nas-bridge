...
Duplicate detection:
    - If an identical file (same SHA-256) already exists, the write is skipped
      and already_existed=True is returned.
    - If the same filename exists but with different content, the new file is
      written with a UTC timestamp suffix (no silent overwrites).
"""
...
def write_file(
    ...
    and ``already_existed=True`` is returned — no error is raised.
    ...
    If the target directory does not exist, HTTP 400 is raised — the workspace
    must be created first via ``POST /api/v1/mkdir``.
